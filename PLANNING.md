...
générateur rend ensuite — 19 intentions, 84 cas de non-régression, portée détaillée dans
son propre README.

| Axe | État |
|---|---|
| **A — structurer la grammaire** | ✅ terminé pour le périmètre visé (v0.16) |
| **B — écrire le traducteur NL** | 🚧 v0.8 — 19 intentions, 84 tests. Déterministe, sans IA à l'exécution (voir ci-dessous) |
...
**Portée actuelle et détail des tranches v0.1 → v0.8** : `traducteur/README.md` (portée
à jour) et `PLANNING_HISTORIQUE.md` section « Axe B » (récit et constats de chaque
tranche).

**Reste à couvrir** : cue lists multiples, cues multipart, patch, courbes, magic sheets,
show control, contrôle partitionné — la majeure partie des 79 actions du modèle. Mark,
Park, Assert, Filtres et Snapshots (v0.5 → v0.8) ne couvrent chacun qu'une forme
