...
| **Journal des questions posées au corpus** — ce que les échanges de session apprennent au projet : trouvaille documentaire, sa source, et surtout sa `consequence` (modèle manquant, absence confirmée, contradiction…). Troisième journal du projet, à côté de `grammar/refus_terrain.yaml` (constats au banc, S) et `traducteur/observations_llm.yaml` (trous de vocabulaire) | [`reference/journal_questions.yaml`](reference/journal_questions.yaml) | 🟡 Ouvert le 2026-09-03, 3 entrées — alimenté à la main via [`PROMPT_QUESTIONS.md`](PROMPT_QUESTIONS.md), jamais de promotion automatique |

## Grammaire exploitable par le code

| Document | Emplacement | Statut |
|---|---|---|
| Modèle Eos typé (objets/actions/matrice de légalité), patrons, générateur IR → commande, injection OSC | [`grammar/`](grammar/README.md) | ✅ v0.22 — 87 actions, 194 règles, 8 paramètres génériques, 142 tests |
| Traducteur français → IR (lexique, intentions, questions) — entre en amont du générateur | [`traducteur/`](traducteur/README.md) | 🚧 v0.20 — 38 intentions, 9 couleurs, 203 + 12 tests, 1415 invariants. Déterministe, sans IA à l'exécution |
