...
| 2026-08-11 | **Badge honnête sur une macro précisée par le moteur flou.** Quand le LLM a tranché une question déjà posée par le lexique, la macro proposée (encore en attente d'envoi) porte un petit indicateur discret qui le dit — jamais laissé ambigu sur l'origine : le corpus reste seul auteur de la syntaxe, le LLM n'a fait que lever une ambiguïté déjà identifiée par le lexique. Disparaît une fois la macro envoyée, comme les hypothèses. | Écran 1 |
| 2026-08-11 | **Journal de renforcement du moteur flou, jamais une promotion automatique.** Chaque trou de vocabulaire rencontré (mot que le LLM ne reconnaît pas, candidat hors options, phrase entièrement incomprise) s'accumule dans une file locale — uniquement si le moteur flou a été explicitement activé, pas de collecte en arrière-plan sinon — exportable en YAML depuis Réglages, prêt à coller dans `traducteur/observations_llm.yaml` après revue. La promotion vers le lexique reste un geste humain, jamais du code : une entrée promue porte une balise `source_llm:` honnête sur son origine, distincte de l'échelle S/A/B/C/D qui mesure autre chose (la fiabilité d'une commande console). | Réglages |
| 2026-08-14 | **Discussion libre à plusieurs tours avec le moteur flou — pour trouver la bonne macro, pas juste répondre à une question déjà posée.** ~~Explicitement demandée (bouton 💬, visible seulement moteur flou activé), pas automatique.~~ *(Le déclenchement a changé le 2026-08-18 — voir plus bas ; le reste de cette décision tient toujours.)* L'IA peut poser des questions en français naturel et proposer plusieurs formulations sur plusieurs tours ; l'utilisateur répond librement, revient en arrière, change d'avis. Le garde-fou central reste entier malgré cette liberté : l'IA ne produit **jamais** de syntaxe Eos — chaque formulation qu'elle propose repasse par le vrai traducteur déterministe, exactement comme si l'utilisateur l'avait tapée lui-même. Un badge distinct (« précisé par une discussion ») documente honnêtement cette origine, différente d'une simple réponse à une question à choix fixes. Contrairement au premier passage silencieux du moteur flou, un échec ici s'affiche dans le fil : l'utilisateur attend activement une réponse. | Écran 1 |
| 2026-08-18 | **La discussion se lance automatiquement sur un incompris, plutôt que sur le bouton 💬 — nuance la décision du 2026-08-14.** Trouvé en testant le beta réel : laisser un incompris silencieux derrière un bouton à cliquer perdait l'intérêt de la discussion pour l'usage réel. Dès qu'une phrase n'est pas comprise et que le moteur flou est configuré (clé API active), la discussion démarre directement avec cette phrase comme premier tour — zéro clic. Le bouton manuel reste affiché seulement quand le moteur flou n'est pas configuré (repli identique à avant). Abandonner une discussion (« Abandonner la discussion ») ne la relance jamais toute seule : le déclenchement automatique ne vit que dans le chemin d'une traduction fraîche, jamais dans celui d'un abandon volontaire. | Écran 1 |
| 2026-08-28 | **Aucun mot de la phrase ne disparaît en silence — et « connu mais inutilisé » n'est pas « pas compris ».** Un mot que la traduction n'utilise pas est signalé sur **tous** les statuts, `compris` inclus : une macro à qui il manque un morceau de la demande a l'air complète, et c'est la classe d'erreur la plus grave du projet (`REGLES_POUR_UI.md`, règle 4). Deux messages distincts pour deux causes distinctes : le mot est absent du lexique, ou il y figure mais la commande ne s'en est pas servie — les confondre faisait annoncer « jaune » comme inconnu alors que l'app venait de le traduire, enseignant à l'utilisateur des limites fausses. Corollaire : plusieurs couleurs dans une phrase de couleur posent désormais une **question** (une commande n'applique qu'une teinte), jamais un choix d'office sur la première. | Écran 1 / Traduction |
| 2026-09-03 | **La Bibliothèque montre d'abord ce que l'app sait traduire — le champ vide ne peut plus être la seule porte d'entrée.** Diagnostic : derrière le champ de saisie, le lexique connaît 23 intentions et une centaine de mots déclencheurs, en correspondance exacte, sans qu'aucun écran ne dise lesquels — l'utilisateur devait deviner (« problème d'habitabilité », documenté depuis quarante ans sur les interfaces en langage naturel : il se corrige en montrant les limites, pas en comprenant mieux). La Bibliothèque porte donc deux sections nettement séparées, parce qu'elles répondent à deux questions différentes : **« Ce que l'app sait traduire »** — une phrase concrète par capacité, cherchable, générée depuis `lexique.yaml` et dont chaque entrée est *garantie traduisible* par un test ; et **« Macros du corpus »** — l'ancienne liste, en syntaxe Eos brute, pas forcément traduisible. Choisir une phrase la dépose dans le composer, modifiable, et elle repasse par le chemin normal : aucun raccourci qui court-circuiterait l'aperçu. Une recherche sans résultat **dit ce qui manque** (« la console sait sans doute le faire, cette app ne sait pas encore le dire ») plutôt que d'afficher une liste vide. | Écran 1 / Bibliothèque |
